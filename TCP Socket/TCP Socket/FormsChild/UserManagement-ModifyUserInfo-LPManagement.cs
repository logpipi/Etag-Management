    ...
    public partial class UserManagement_ModifyUserInfo_LPManagement : Form
    {
        ...
        private void RightClickDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show("確認刪除？", "提醒", MessageBoxButtons.YesNo).Equals(DialogResult.Yes))
            {
                ...
                conditionInfo.Add("Card_number='" + updateInfo[0]+ "'");
                if (sql.Update("Menbers_Table", upInfo, conditionInfo))
                {
                    listViewShow.Items.Remove(listViewShow.Items[listviewIndex]);
                }
            }
        }
        ...
        {
            DataTable dt = sql.GetExcelDatatable("C:\\Users\\H310M\\Desktop\\a.xlsx");
            sql.InsetData(dt);
        }
